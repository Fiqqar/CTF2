...
# 2. Mapping kebalikan (Warna -> Simbol BF)
inverse_mapping = {
    (255, 0, 0): '+',   (0, 255, 0): '-',
    (0, 0, 255): '>',   (255, 255, 0): '<',
    (255, 0, 255): '[', (0, 255, 255): ']',
...
# 3. Ekstrak kode BF dari baris pertama
bf_code = ""
for x in range(width):
    color = pixels[x, 0][:3] # Ambil RGB aja
    if color in inverse_mapping:
        bf_code += inverse_mapping[color]

print(f"Extracted BF: {bf_code}\n")

# 4. Interpreter Brainfuck Sederhana
def brainfuck(code):
    tape = [0] * 30000
    ptr = 0
    pc = 0
    ...
    # Pre-proses untuk nyari pasangan bracket [ ]
    stack = []
    jump_map = {}
